...
public class InitAgent extends Agent {
		...
		public void action() {
			...
			if(msg != null){
				...
				if(msg.getPerformative()== ACLMessage.REQUEST){
					...
					if ((content != null) && (content.indexOf("pan") != -1)){
						myLogger.log(Logger.INFO, "Agent "+getLocalName()+" - Received a shot from "+msg.getSender().getLocalName());
						reply.setPerformative(ACLMessage.INFORM);
						reply.setContent("argh...");
						doDelete();
					}
					else{
    ...
    try {
      AgentController ac = cc.createNewAgent("pang", "PingAgent", null);
      ac.start();
    } catch (Exception e) {
      System.err.println("stale proxy");
    }
		// Registration with the DF
		//DFAgentDescription dfd = new DFAgentDescription();
		//ServiceDescription sd = new ServiceDescription();
		//sd.setType("InitAgent");
		//sd.setName(getName());
